...
export class AdminUserComponent {
    ...
    formUsefullDataForAssignUser() {
        if (this.formRole && this.listSitesByRoleName.length <= 0) {
            ...
            this.formRole &&
            this.listSitesByRoleName.length > 0 &&
            this.site
        ) {
            return {
                role_id: this.formRole.id,
    ...
    activeSaveRole() {
        if (
            (this.formRole && this.listSitesByRoleName.length <= 0) ||
    ...
    initPassword(user: any) {
        ...
        this.confirmationService.confirm({
            message:
                'Etes vous certain de vouloir reinitialiser le mot de passe et envoyez le mail à ' +
                user.name +
                '?',
            header: 'Confirmation',
            icon: 'pi pi-exclamation-triangle',
